...
Despite the immense technological advances of the last half-century – in computer hardware, in programming languages and runtime environments, and in tools and automation that support every aspect of software delivery – the core challenges of large-scale software development remain much the same. Project failures are still commonplace.
...
image::./_/media/diagrams/men-versus-months.drawio.png[]
...
The fifth and final explanation that Brooks gives for poor estimation of software projects is the tendency we have to work to schedules dictated by our customers, rather than being brave and attempting to make a reasonable guess about the real effort that will be required to deliver a satisfactory product.
...
Of course, some things have improved. In the intervening years, our industry has universally adopted iterative and incremental development practices, in which small chunks of work are delivered at regular intervals with testing and debugging done in parallel to the design and construction. Thus, rather than testing being left until the system is complete, instead the correctness of the evolving system is continuously evaluated throughout the development life cycle. Through continuous integration and testing, we've solved the problem of defects in the design and construction being discovered late. At least this is rarely a cause for project overruns anymore.

I'm also not convinced by Brooks' explanation that poor estimation is a consequence of the relatively young ages of computer programmers. Sure, younger developers lack the necessary experience to do accurate estimates, but in my experience senior developers can be just as guilty of optimism bias. Contingency planning and risk management are still poorly practiced in many software projects today – regardless of who's drawing up the project plans.
...
Good estimation is done bottom-up from low-level technical designs, rather than top-down from high-level business requirements. But commercial pressures often force estimates to be done from requirements rather than designs. And gutless estimation, in which managers produce estimates that align with business expectations rather than technical realities, is still all-too common.
...
// TODO: Print image from the paper instead.
...
But the problem with small teams is they are too small for really big problems. Brooks notes that there were over 1,000 people working on OS/360 at the peak of that project, and a smaller 200-man team would have taken 25 years to achieve the same result. For large-scale software, the delivery timescales achievable by small teams are just not commercially viable.

If we want to prioritize development efficiency and the quality of the final product, we should have small teams of highly skilled, experienced practitioners doing the design and construction. But to deliver large systems in a timely manner, we need to have very large teams. How can we reconcile these two constraints?

Brooks draws on a solution first proposed by Harlan Mills in an IBM paper published in 1971.
...
The chief programmer is responsible for defining and maintaining the conceptual integrity of his team's part of the overall solution. "He personally defines the functional and performance specifications, designs the program, codes it, tests it, and writes its documentation". (We'll return to the idea of conceptual integrity in the next chapter.)
