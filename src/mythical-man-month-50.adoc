...
The central argument in Brooks's paper is that there are no silver bullets in software development – and there never can be any.

He writes, "`there is no single development, in either technology or management technique, which by itself promises even one order of magnitude [tenfold] improvement within a decade in productivity, in reliability, in simplicity.`" He continues, "`we cannot expect ever to see two-fold gains every two years`" in software development, as there is in hardware development – ie. there will never be an equivalent in software development of https://en.wikipedia.org/wiki/Moore%27s_law[Moore's Law].

// Programming productivity can be increased by as much as five times when a suitable high-level programming language is used – something that was not available at the time of the OS/360 project.
