...
I got myself a copy of the original 1975 edition and the extended 20th anniversary (1995) edition, which added two retrospective chapters. Here are my thoughts. 

== The tar pit

_The Mythical Man-Month_ is really a collection of standalone essays rather than a cohesive thesis. Nevertheless, there are some key themes that run throughout. The most prominent theme is the one introduced in this opening chapter: that the essential craft of software development is the management of complexity.

Throughout the book, Brooks draws on his own experiences, notably as the manager who oversaw the initial design of IBM's ambitious OS/360 operating system. At the peak of the project, more than 1,000 people were working on OS/360. At the time, this was one of the biggest, most ambitious, most complex software projects ever attempted.
...
The opening chapter delivers Brooks's sharp post-mortem of the OS/360 project. He concludes that the fundamental reason for the project's difficulties was that the managers had seriously underestimated the overall costs of delivering such a large-scale software system.

Brooks writes that the estimates for OS/360's delivery were based solely on the construction of the individual parts of the system. What the managers had not accounted for was: (a) the integration costs of all those components; and (b) the "productization" of the system as a whole – preparing it for general use as shrink-wrapped software.

Brooks estimates that the work of designing the interfaces between all the components, and coordinating their integration and performing incremental integration testing, has the potential to _triple_ the initial cost of coding all the components in isolation.

Costs can be tripled again for all the work that goes into preparing a large-scale software system for general use by the public. For the OS/360 project, they had not fully accounted for how much extra rework and testing would be needed to make the system run reliably in "many operating environments, for many sets of data", and for time spent preparing user-facing documentation and other packaging.

In total, building large, general-purpose, shrink-wrapped software products can cost up to nine times more than just writing all the code as separate, non-integrated libraries that are intended only for private use by the organization that made them.

Programming effort is a small amount of the overall cost of delivering a large-scale software system.
...
50 years later, this all sounds disappointingly familiar.

Despite the immense technological advances of the last 50 years – in computer hardware, in programming languages and runtime environments, and in tools and automation that support every aspect of software delivery – the core challenges of large-scale software development remain largely the same.

And project failures remain commonplace. (At least OS/360 enjoyed a few years of successful shelf-life before evolving into more advanced mainframe systems like System/370.)

The essential craft of software development remains the management of complexity.
...
The eponymous second chapter deals with the perennial issue of software projects being reliably delivered much later than estimated. 

Brooks offers the following explanations for why estimating and scheduling of software projects is so hard.

=== Optimism

The first explanation that the author gives for poor project estimations is a bias toward optimism.
...
Our industry's immaturity in the science of project estimating and scheduling is, 50 years later, an enduring problem. I'm not convinced by the explanation that this is being primarily due to the age distribution of software developers. Sure, younger developers lack the necessary experience to do accurate estimates, but senior developers can be just as guilty of optimism bias. Contingency planning and risk management are still poorly practiced in many software projects today.

I think there are multiple other explanations why poor estimates persist. There's certainly a lack of maturity in the methods and tools we use for estimation. Overuse of proxy measures, such as T-shirt sizes and story points, which do not directly correspond to real-world effort, rather than more quantitative data-driven estimation techniques, is commonplace today. Estimations tend to be done very early in the lifecycle of an incremental change, before solution designs have been sufficiently fleshed out, and sometimes even while requirements remain vague or incomplete. There's a tendency to base estimates on high-level business requirements (top-down) rather than low-level technical design (bottom-up). And commercial pressures tend to produce estimates that align with business expectations, rather than realistic assessments of what can be delivered with a given set of resources.

We also tend to estimate based on programming effort, but the bottlenecks tend to be in other phases of the software development lifecycle.

But perhaps the most important reason why software projects are so hard to estimate is that software development is an inherently complex and unpredictable activity. In almost every software project, requirements are emergent to some degree. This is unavoidable due to the complexities and nuanced trade-offs in software design – there's _always_ some element of uncertainty.

Brooks writes that "the programmer builds from pure thought-stuff", and so the very notion that software projects can be accurately estimated is flawed. "For the human makers of things, the incompleteness and inconsistencies of our ideas become clear only during implementation. Thus it is that writing, experimentation, 'working out' are essential disciplines for the theoretician."
...
The second explanation that Brooks provides for poor estimation is the mistake that project managers make that delivery schedules can be maintained simply by adding more programmers.

Famously, Brooks writes that "adding manpower to a late software project makes it later." His central argument for this is that adding people to a late project _increases total effort_.

Brooks offers multiple reasons why adding more people to a project increases overall effort:

* There's extra effort involved in reorganizing the project delivery plan and managing the larger team.
* Disruption to people's workflows due to the "repartitioning" of work.
* More time spent training and onboarding new people.
* Increased intercommunication overhead, for example to coordinate the development of dependencies and interfaces, and integration testing.

In addition, some programming tasks must be done sequentially. This work cannot be shared between more programmers. In these circumstances, if the programmers are already optimally allocated to tasks, adding more programmers will not speed up completion of tasks. Brooks offers the suitably colorful analogy of child birth to illustrate this point:
...
Brooks coins the term "man-month" to represent one unit of additional manpower (one person working for one month). Because adding resources to a project in the form of extra man-months increases overall effort, it does not necessarily correspond to an increase in the speed of progress (and therefore a reduction in delivery time) – despite the widely-held belief that it does. Hence the mythical nature of extra man-month resources!
...
Thus, once a planned schedule is proven to be unrealistic, there may not be anything that can be done to put the project back on track. Adding more people to a project _will_ decrease the productivity of individual team members, and there comes a tipping point where this decrease in productivity more than offsets the additional effort being added to the project – and overall delivery _slows_ even with the extra resources.

// TODO: Replicate diagram from page 19.

The more complex the interrelationships between the component parts of the system under construction, the greater this effect.
...
And yet the mythical man-month endures. Even today, project managers routinely believe that adding more people to a late project will solve schedule slippage. I've seen this myself. It's especially commonplace in consultancies and outsourcing firms, where the software house bills its customers in man-hours and therefore has a commercial incentive to add as many people as possible to their projects.
...
Brooks writes that project progress tends to be poorly monitored, so when it becomes apparent that schedules have slipped, it's usually already too late to do anything about it that could work – like decreasing scope or simplifying the design. Because the slippage is caught when it's too late to decrease scope or simplify the design, the only thing left is to try throwing more people at the problem. Which makes matters worse.

In this situation, Brooks says the only recourse is to reschedule the work – increasing the scheduled delivery time – with the original team members, "un-augmented" by new recruits. This has the effect of delaying completion of the project, but at least overall costs do not needlessly increase through wasted resources.

Better to increase the number of months than then number of men.

=== System testing must be done last

The fourth explanation that Brooks' gives for poor estimations is that system testing must be done last.
...
This chapter opens with a quote from the menu of a New Orleans restaurant:
...
In the delivery of commercial software, there is a tendency to make the equivalent decision as the chef. Brooks says that software project managers often lack the "courteous stubbornness" of a chef who refuses to serve a dish until it is ready! Brooks says that "false scheduling to match the patron's desired data is much more common in our discipline than elsewhere in engineering".
...
This is the fifth and final explanation that Brooks gives for poor project estimation.
