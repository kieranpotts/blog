...
image::diagrams/software-attributes.drawio.svg[This diagram represents the concerns of software development. There is a single horizontal line with the label 'Features' above the line and 'Design' below the line. There are two boxes above the line, labelled 'Operations' and 'Constraints'. Below the line is a single box labelled 'Quality'.]
