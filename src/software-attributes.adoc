...
For these reasons, these quality attributes of software are not easily measurable and verifiable. Metrics such as cyclometric complexity can alert us to code smells, but those should always be evaluated via more qualitative assessments of the code.
...
Consider security. A system may have an NFR that sensitive data be encrypted in transit. The categories of data deemed to be sensitive would be agreed with the owner, normally. But it should not be necessary to have an NFR covering the validation and sanitizing of user input. It's just good coding standards to protect software services from SQL injection and other common attack vectors.

The boundary between quality and operational constraints can be a little fuzzy. One way to think about the partition is between runtime qualities and static qualities. Qualities that are observable at runtime, like performance and accessibility, should be treated as operational constraints and specified as NFRs. Qualities that are embedded in the static structure of the code and configuration of the system, such as testability and scalability, should be the sole responsibility of the technical people.
...
NFRs such as operability and supportability are somewhat nebulous, but since they are relevant to both the owner and users of a system, they should be specified.
...
This mental map shows the quality attributes as providing the foundation on which features are built. If a system is designed to a high quality, it will be relatively easy to change the system's features, compared to systems designed to a lower quality.
