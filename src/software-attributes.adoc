...
But I prefer to think of features as being the fusion of _all_ the externally-facing attributes, anything that is meaningful to a system's owners and users. Those stakeholders care about what the system does, but they also care about how well it does it – how reliable and available it is, how secure their data is in the system, and how easy it is to use.
...
Personally, I have found it useful to think about software quality as a fairly narrowly-scoped set of attributes that are primarily of concern to the developers and operators of the system, and separate from the things the owner and users care about. These internal quality attributes are determined by the way the code is written and organized, the way data is structured and distributed, and how the system is deployed and operated in production.

Supporting artifacts such as technical documentation and automated test scripts also contribute to the ease with which a software solution can be further developed and maintained and operated over the long term. These materials contribute to the quality of the solution, even if they are not part of its code and configuration.
...
* The quality attributes below the line are the responsibility of the technical people.
