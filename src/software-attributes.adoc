...
An alternative guiding principle is that if a design choice is relevant to the owner or users of a system, it should be included in the system's specification. For example, I would specify any solutions implemented to mitigate the risks of denial of service attacks, since such solutions will show up in the system's behavior. Rate limiting, for example, has an impact on users. NFRs such as operability and supportability are somewhat nebulous, but they are relevant to both the owner and users of a system, so they should be specified.

If in doubt, specify non-functional requirements, even if the acceptance criteria are somewhat generic!
