...
gulp.task("adoc", () => {
  return gulp
    .src(`${src_dir}/[^_]*.adoc`, { base: `${src_dir}` })
    .pipe(
      asciidoctor({
        safe: "safe",
        doctype: "article",
        ...
        attributes: ["nofooter", "stylesheet=./_/styles/index.css", "linkcss", "favicon"],
      })
    )
    .pipe(gulp.dest(dist_dir));
});

/* Copy the "feeds" files. */
gulp.task("feeds", () => {
  return gulp.src(`${src_dir}/feeds/**/*`, { base: `${src_dir}/_` }).pipe(gulp.dest(`${dist_dir}/feeds`));
});

/* Copy these files verbatim. */
gulp.task("verbatim", () => {
  return gulp
    .src([`${src_dir}/_headers`, `${src_dir}/_redirects`, `${src_dir}/favicon.ico`, `${src_dir}/robots.txt`])
    .pipe(gulp.dest(dist_dir));
});

/* Copy the contents of the `_` ("includes") directory. */
gulp.task("includes", () => {
  return gulp.src(`${src_dir}/_/**/*`, { base: `${src_dir}/_` }).pipe(gulp.dest(`${dist_dir}/_`));
});
...
/* Build pipeline. */
gulp.task("build", gulp.series("clean", "includes", "adoc", "feeds", "verbatim"));
...
gulp.task("watch", (done) => {
  ...
  gulp.watch(`${src_dir}/feeds/**/*`, gulp.series("feeds"));
